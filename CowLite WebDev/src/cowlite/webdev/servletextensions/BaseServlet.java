...
import cowlite.webdev.session.Session;
import cowlite.webdev.session.SessionManager;
import cowlite.webdev.session.User;
...
/**
 * This class represents the base-servlet which should be extended by all
 * other servlet classes. It provides basic functionality such as the sending
 * of error messages, requesting SQL-statement executions and sending replies.
...
{
    /**
     * DataSource class that manages the connections towards the DB.
     */
    @Resource(name = "jdbc/Exhale")
    ...
    public String getServletInfo() {
        return "Author: Wessel Jongkind. Version 1.0, date 29-06-2017.";
    }
     ...
     * if the request has been made from http://www.something.com/map1/index.html then this
     * method will return http://www.something.com.
     ...
     * @param moduleID The ID of the module to which the requested resource belongs.
     * @param modules An ArrayList with all modules that the user should have acces to.
    ...
    protected boolean verifySession(HttpServletRequest request, Session session) {
        /*
            Either the session can't be found (expired) or the IP changed
        */
        if(session == null ||
           (!session.getIP().equals(request.getRemoteAddr())))
            return false;
        else
        {
     ...
     * @param request The HTTP request of which the associated Session-object should be retrieved.
     * @return The Session-object associated with the HTTP request.
     */
    protected Session getSession(HttpServletRequest request)
    {
        String sessionID = getCookieValue(SessionManager.SESSION_ID, request);
    ...
    public void setJSONHeaders(HttpServletResponse response) {
